...
const updateReplyCommentController = async(req, res, next) => {
    ...
    try{
        ...
        if(comment.replies[replyIndex].user.toString()!==userId){
            throw new CustomError("You can only update your comments", 400)
        }
...
const populateUserDetails = async(comments)=>{
    for(const comment of comments){
        await comment.populate("user", "username fullName profilePic")
        if(comment.replies.length > 0){
            await comment.populate("replies.user", "username fullName profilePic")
        }
    }
}
...
const likeCommentController = async(req, res, next) => {
    ...
    try{
        ...
        res.status(200).json({message: "Comment likes successfully!", comment})
    }
    catch(error){
        next(error)
    }
...
const likeReplyCommentController = async(req, res, next) => {
    ...
    try{
        ...
        if(replyComment.likes.includes(userId)){
            throw new CustomError("You have already liked this reply comment", 400)
        }
        replyComment.likes.push(userId)
        await comment.save()
        res.status(200).json({message: "Reply comment liked successfully!", replyComment})
    }
    catch(error){
        next(error)
    }
...
const dislikeReplyCommentController = async(req, res, next) => {
    ...
    try{
        ...
        if(!replyComment.likes.includes(userId)){
            throw new CustomError("You have not liked this reply comment", 400)
        }
        replyComment.likes=replyComment.likes.filter(id=>id.toString()!==userId)
        await comment.save()
        res.status(200).json({message: "Reply comment disliked successfully!", replyComment})
    }
    catch(error){
        next(error)
    }
